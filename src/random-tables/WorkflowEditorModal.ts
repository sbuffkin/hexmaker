...
export class WorkflowEditorModal extends HexmakerModal {
  ...
  async onOpen(): Promise<void> {
    ...
    const rawContent =
      this.preloaded?.content ?? (await this.app.vault.read(this.file));
    ...
    // ── Template section ──────────────────────────────────────────────
    const templateSectionWrap = contentEl.createDiv({
      cls: "duckmage-wf-template-section-wrap",
    });
    templateSectionWrap.createEl("p", {
      text: "Template",
      cls: "duckmage-table-editor-heading",
    });
    const templateArea = templateSectionWrap.createEl("textarea", {
      cls: "duckmage-wf-template-area",
    });
    templateArea.value = templateContent;
    templateArea.addEventListener("input", () => {
      templateContent = templateArea.value;
      updateValidation();
    });
    ...
    const datalistId =
      "duckmage-rf-folders-" + Math.random().toString(36).slice(2);
    ...
    const rfInput = rfRow.createEl("input", {
      type: "text",
      cls: "duckmage-table-editor-name-input",
    });
    ...
    rfInput.addEventListener("input", () => {
      resultsFolder = normalizeFolder(rfInput.value.trim());
    });
    ...
    const updateValidation = () => {
      ...
      if (missing.length === 0) {
        ...
        validationEl.setText(
          required.length > 0 ? "✓ All placeholders present" : "",
        );
      } else {
    ...
    const renderSteps = () => {
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (step.kind === "dice") {
          ...
          tableSelect.createEl("option", {
            value: "",
            text: "— select table —",
          });
          for (const tf of tableFiles) {
            const val = tf.path.slice(0, -3);
            const optLabel = tablesFolder
              ? tf.path.slice(tablesFolder.length + 1, -3)
              : val;
            const opt = tableSelect.createEl("option", {
              value: val,
              text: optLabel,
            });
          ...
          if (
            step.tablePath &&
            !tableFiles.some((tf) => tf.path.slice(0, -3) === step.tablePath)
          ) {
            const opt = tableSelect.createEl("option", {
              value: step.tablePath,
              text: step.tablePath,
            });
            opt.selected = true;
          }
          tableSelect.addEventListener("change", () => {
            ...
            const newAutoLabel = labelFromBasename(
              tableSelect.value.split("/").pop() ?? tableSelect.value,
            );
            const oldLabel = prevLabel;
            labelInput.value = newAutoLabel;
            steps[i].label = newAutoLabel || undefined;
    ...
    // Type selector
    const addTypeSelect = addRow.createEl("select", {
      cls: "duckmage-wf-add-type-select",
    });
    addTypeSelect.style.cssText =
      "-webkit-appearance:none;appearance:none;padding:2px 16px 2px 4px;" +
    ...
    const addSpecificWrap = addRow.createDiv({
      cls: "duckmage-wf-add-specific",
    });
    ...
    const buildAddSpecific = () => {
      ...
      if (addTypeSelect.value === "dice") {
        ...
        addTableSelect.createEl("option", {
          value: "",
          text: "— select table —",
        });
        for (const tf of tableFiles) {
        ...
        addTableSelect.addEventListener("change", () => {
          if (!addLabelInput.value.trim() && addTableSelect!.value) {
            const basename =
              addTableSelect!.value.split("/").pop() ?? addTableSelect!.value;
    ...
    const addBtnRow = addArea.createDiv({
      cls: "duckmage-wf-editor-add-btn-row",
    });
    ...
    addBtn.addEventListener("click", () => {
      ...
      if (addTypeSelect.value === "dice") {
        ...
        if (!isDiceFormula(formula)) {
          new Notice("Invalid dice formula — use format like 2d6+6");
          return;
        }
        ...
        newStep = {
          ...
          diceFormula: formula,
          rolls,
          label: rawLabel || autoLabel,
        };
        addFormulaInput!.value = "";
      } else {
        const tablePath = addTableSelect?.value ?? "";
        if (!tablePath) return;
        const autoLabel = labelFromBasename(
          tablePath.split("/").pop() ?? tablePath,
        );
        newStep = {
          kind: "table",
          tablePath,
    ...
    this.flushAndSave = async () => {
      ...
      try {
        await this.app.vault.process(this.file, () =>
          buildWorkflowContent(updatedWorkflow),
        );
