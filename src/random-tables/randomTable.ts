...
export function parseRandomTable(content: string): RandomTable {
  ...
  const preambleText = firstTableLine
    ? afterFm.slice(0, firstTableLine.index)
    : "";
  ...
  for (const line of lines) {
    ...
    if (result)
      entries.push({ result, weight, ...(isLink ? { isLink: true } : {}) });
  }
...
export function getDieRanges(table: RandomTable): string[] {
  ...
  const floored = table.entries.map((e) =>
    Math.max(1, Math.floor((e.weight / total) * faces)),
  );
  let used = floored.reduce((s, v) => s + v, 0);

  // Distribute remaining faces by fractional remainder, largest first
  if (used < faces) {
    const remainders = table.entries
      .map((e, i) => ({
        i,
