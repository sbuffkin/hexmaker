...
describe("parseRandomTable", () => {
  ...
  it("parses linkedFolder from frontmatter", () => {
  ...
  it("trims whitespace from linkedFolder value", () => {
    const content = `---\ndice: 6\nlinkedFolder:   world/towns  \n---`;
    expect(parseRandomTable(content).linkedFolder).toBe("world/towns");
  });
  ...
  it("sets isLink=true for wiki-link cells in a non-linkedFolder table", () => {
...
describe("getDieRanges", () => {
  ...
  it("total faces used always equals dice value", () => {
    ...
    const lastFace = parseInt(
      lastRange.includes("–") ? lastRange.split("–")[1] : lastRange,
    );
